...
```{r Figure6-16,fig.cap = "(ref:fig6-16)",echo = F}
...
```{r echo = F}
# Equation for log-hectares with degrees canceled
...
```{r Figure6-19,fig.cap = "(ref:fig6-19)",echo = F,results = "hold"}
m1 <- lm(BAC ~ Beers,data = BB)

par(mfrow = c(1,3))
plot(BAC ~ Beers,ylim = c(-0.15,.35),xlim = c(1,9),pch = 16,data = BB,main = "Real Data")
abline(a = m1$coef[1],b = m1$coef[2],col = "darkgreen", lwd = 2)

BBfake1 = BB
BBfake1$BAC = fitted(m1)+residuals(m1)*5
plot(BAC ~ Beers,ylim = c(-0.15,.35),xlim = c(1,9),pch = 16,data = BBfake1,main = "Fake Data")
m1 <- lm(BAC ~ Beers,data = BBfake1)
abline(a = m1$coef[1],b = m1$coef[2],col = "darkgreen", lwd = 2)

BBfake2 = BB
BBfake2$BAC = fitted(m1)+residuals(m1)/15
plot(BAC ~ Beers,ylim = c(-0.15,.35),xlim = c(1,9),pch = 16,data = BBfake2,main = "Fake Data")
...
```{r Figure6-21,fig.cap = "(ref:fig6-21)",echo = F,warning = F,message = F, fig.height=12, out.width="100%"}

BBfake3 <- as.data.frame(BB)
BBfake3 <- rbind(BBfake3,c(11,0.01))
...
BBfake4 <- as.data.frame(BB)
BBfake4 <- rbind(BBfake4,c(11,0.19))
...
BBfake5 <- as.data.frame(BB)
BBfake5 <- rbind(BBfake5,c(5,0.19))
...
BBfake6 <- as.data.frame(BB)
BBfake6 <- rbind(BBfake6,c(5,0.01))
...
* **Beers vs BAC:**
    ...
    * Independence of observations:
        ...
        * We can assume that all the subjects are independent of each 
        ...
        ```{r Figure6-22,fig.cap = "(ref:fig6-22)"}
        m1 <- lm(BAC ~ Beers, data = BB)
        par(mfrow = c(2,2))
...
```{r Figure6-23,fig.cap = "(ref:fig6-23)",echo = F}
...
```{r Figure6-24,fig.cap = "(ref:fig6-24)"}
...
```{r Figure6-25,fig.cap = "(ref:fig6-25)"}
par(mfrow = c(2,2))
plot(OF1)
```
...
* **`r colFmt("DATASETNAME","red")` |> ggpairs()**
...
geom_smooth(method = "lm")**
    ...
    * Add **color = `r colFmt("groupfactor","red")`** to the **aes()** to color
