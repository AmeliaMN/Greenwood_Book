...
```{r Figure8-1,fig.cap="(ref:fig8-1)"}
library(GGally)
...
```{r Figure8-2,fig.cap="(ref:fig8-2)",echo=F}
library(effects)
...
```{r Figure8-3,fig.cap="(ref:fig8-3)",echo=F}
...
```{r Figure8-4,fig.cap="(ref:fig8-4)",echo=F}
...
```{r Figure8-5,fig.cap="(ref:fig8-5)"}
...
```{r Figure8-6,fig.cap="(ref:fig8-6)"}
...
```{r Figure8-7,fig.cap="(ref:fig8-7)"}
...
```{r echo=F}
snotel_s$Station <- as.character(snotel_s$Station)
snotel_s$Station[snotel_s$Station == "Bloody Dick"] <- "Bloody [Redact.]"
snotel_s$Station <- factor(snotel_s$Station)
```
...
```{r Figure8-8,fig.cap="(ref:fig8-8)"}
par(mfrow = c(2,2), oma = c(0,0,2,0))
...
```{r Figure8-9,fig.cap="(ref:fig8-9)",echo=F}
...
```{r Figure8-10,fig.cap="(ref:fig8-10)"}
...
```{r Figure8-11,fig.cap="(ref:fig8-11)",fig.height=12,out.width="100%"}
...
```{r echo=F}
n<-100; set.seed(9754)
x1<-runif(n = 100,0,100); x2<-runif(n = 100,0,100); 
x3 <- (runif(n = 100,0,100) + x1); x3 <- 100*(x3-min(x3))/(max(x3) - min(x3))
...
```{r Figure8-13,fig.cap="(ref:fig8-13)",echo=T}
...
```{r Figure8-14,fig.cap="(ref:fig8-14)",echo=T}
...
```{r Figure8-15,fig.cap="(ref:fig8-15)",echo=T}
...
* **Be cautious about doing prediction in MLR -- you might be doing extrapolation!**
    ...
    ```{r Figure8-16,fig.cap="Scatterplot of observed Elevations and Maximum Temperatures for SNOTEL data.",echo=F}
    plot(Max.Temp ~ Elevation, data = snotel_s |> slice(-c(9,22)), pch = 16,
...
* **Multicollinearity in MLR models:**
    ...
    ```{r Figure8-17,fig.cap="Plot of correlation matrix in the snow depth data set with influential points removed",echo=F}
    library(corrplot)
    par(mfrow = c(1,1), oma = c(0,0,1,0))
    corrplot.mixed(cor(snotel_s |> slice(-c(9,22)) |> select(3:6)), 
                   upper.col = c(1, "orange"), lower.col = c(1, "orange"))
...
\indent `r textItalicBold("VIFs")` provide a way to assess the
...
```{r Figure8-18,fig.cap="(ref:fig8-18)"}
library(openintro)
data(satgpa)
...
```{r Figure8-19,fig.cap="(ref:fig8-19)"}
...
```{r Figure8-21,fig.cap="(ref:fig8-21)"}
...
```{r Figure8-23,fig.cap="(ref:fig8-23)",echo=F}
satgpa |> ggplot(mapping = aes(x = satm, y = fygpa, color = GENDER, shape = GENDER)) +
  geom_smooth(method = "lm") +
  geom_point(alpha = 0.7, size = 2.5) +
  theme_bw() +
...
par(mfrow = c(1,1))
plot(dm1$satm,fitmales,ylim = c(min(fitmales),max(fitfemales)),
     main = "Plot of estimated regression lines by gender",
     type = "l",lwd = 2,xlab = "satm",ylab = "fygpa", col = inferno(8)[1])
lines(dm1$satm,fitfemales,lwd = 3,col = inferno(8)[7])
abline(v = 50,lty = 2,col = "grey")
abline(h = 0.216+0.0386*50,lty = 2,col = inferno(8)[1])
abline(h = 0.529+0.0386*50,lty = 2,col = inferno(8)[7])
legend("topleft", c("Male", "Female"),lwd = c(2,3),lty = 1,col = inferno(8)[c(1,7)])
```
...
```{r Figure8-26,fig.cap="(ref:fig8-26)"}
...
```{r Figure8-27,fig.cap="(ref:fig8-27)",echo=F}
...
plot(du1s,predict(head1, newdata = hdcontrol), lwd = 2, lty = 2, ylim = c(0,16),
     ...
     main = "Plot of additive estimated regression model", col = inferno(5)[1])
lines(du1s, predict(head1, newdata = hdT1), lwd = 2, col = inferno(5)[2])
lines(du1s, predict(head1, newdata = hdT2), lwd = 2, col = inferno(5)[3], lty = 2)
lines(du1s, predict(head1, newdata = hdT3), lwd = 3, col = inferno(5)[4], lty = 3)
legend("topleft", lwd = c(2,2,2,3), title = "Groups",
       legend = c("Control","T1","T2","T3"), lty = c(2,1,2,3), col = inferno(5)[-5])
```
...
```{r Figure8-29,fig.cap="(ref:fig8-29)"}
...
```{r Figure8-31,fig.cap="(ref:fig8-31)",echo=F}
library(smdata)
data("dyslexic3")
dyslexic3 <- dyslexic3 |> mutate(dys = as_factor(dys)) |>
  mutate(dys = fct_recode(dys, no = "0", yes = "1"))
...
```{r Figure8-32,fig.cap="(ref:fig8-32)"}
par(mfrow = c(2,2), oma = c(0,0,2,0))
...
```{r Figure8-38,fig.cap="(ref:fig8-38)",echo=F}
...
```{r Figure8-40,fig.cap="(ref:fig8-40)",echo=F,warning=F,message=F}
...
```{r Figure8-41,fig.cap="(ref:fig8-41)",echo=F,warning=F,message=F}
...
```{r Figure8-42,fig.cap="(ref:fig8-42)",echo=F, fig.height=9}
library(coneproj)
data(FEV)
FEV <- as_tibble(FEV)
...
```{r Figure8-43,fig.cap="(ref:fig8-43)"}
...
```{r Figure8-44,fig.cap="(ref:fig8-44)"}
...
```{r Figure8-45,fig.cap="(ref:fig8-45)",echo=F,warning=F,message=F}
...
```{r Figure8-46,fig.cap="(ref:fig8-46)"}
coefficients(fm1R)
vif(fm1R)
confint(fm1R)
...
* **`r colFmt("DATASETNAME","red")` |> ggplot(mapping = aes(x = `r colFmt("x","red")`, y = `r colFmt("y","red")`)) +  
geom_point() +  
geom_smooth(method = "lm")**
    ...
    * Add **color = `r colFmt("groupfactor","red")`** to aesthetic to color
    points and lines based on a grouping variable. 
    
    * Add **+ facet_grid(cols = vars(`r colFmt("groupfactor","red")`))** to
...
* **`r colFmt("MODELNAME","red")` ``<-`` lm(`r colFmt("y","red")` ~ 
`r colFmt("x1 * groupfactor","red")`, data = `r colFmt("DATASETNAME","red")`)**
