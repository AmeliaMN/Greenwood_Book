...
```{r Figure4-2,fig.cap="(ref:fig4-2)"}
...
```{r Figure4-3, fig.cap="(ref:fig4-3)"}
...
```{r Figure4-4, fig.cap="(ref:fig4-4)", fig.height=9}
...
```{r Figure4-5, fig.cap="(ref:fig4-5)",echo=F}
...
```{r Figure4-6, fig.cap="(ref:fig4-6)",echo=F}
...
```{r Figure4-7,fig.cap="(ref:fig4-7)",echo=T}
library(effects)
...
pirateplot(len ~ supp * dose, data = ToothGrowth, ylim = c(0,35),
         ...
         inf.method = "ci", inf.disp = "line", theme = 2, point.o = .75)
...
```{r Figure4-11,fig.cap="Diagnostic plots for the interaction model for odontoblast growth interaction model."}
par(mfrow = c(2,2))
plot(TG2, pch = 16) 
```
...
\index{validity conditions!Two-Way ANOVA}
    ...
    * Constant variance:
        ...
        * In the Residuals vs Fitted and the Scale-Location plots, the
        ...
        ```{r Figure4-12,fig.cap="Term-plot for odontoblast growth interaction model with partial residuals added."}
...
```{r Figure4-14,fig.cap="(ref:fig4-14)",echo=F}
par(mfrow = c(1,1))
...
```{r Figure4-15,fig.cap="(ref:fig4-15)", fig.height=9}
...
```{r Figure4-16,fig.cap="(ref:fig4-16)"}
par(mfrow = c(2,2))
plot(debt1, pch = 16)
```
...
\index{validity conditions!Two-Way ANOVA}
    ...
    ```{r Figure4-17,fig.cap="Diagnostic plot of additive model for ``prodebt`` by income group and whether they buy cigarettes/not."}
    ...
    * Constant Variance:
        ...
        * We can also explore the partial residuals here as provided in
        ...
        ```{r Figure4-18,fig.cap="Term-plot for additive model for ``prodebt`` by income group and whether they buy cigarettes/not with partial residuals."}
...
5. **Size:**
    ...
    * Figure \@ref(fig:Figure4-19) shows the increasing average comfort with being in
    ...
    ```{r Figure4-19,fig.cap="(ref:fig4-19)"}
...
```{r Table4-3b,echo=F,results="asis"}
...
m <- as.data.frame(t)
colnames(m) <- columnHeaders
pandoc.table(m, caption = "(\\#tab:Table4-3) (ref:tab4-3)",
             split.table = Inf, justify = "left", keep.line.breaks = TRUE)
```
...
```{r Figure4-20,fig.cap="(ref:fig4-20)",echo=F}
par(mfrow = c(1,2))
...
```{r Figure4-21,fig.cap="(ref:fig4-21)",echo=F}
par(mfrow = c(1,1))
...
```{r Figure4-22,fig.cap="(ref:fig4-22)"}
...
```{r Figure4-23,fig.cap="(ref:fig4-23)",echo=T}
library(tabplot)
...
* **`r colFmt("DATASETNAME","red")` <- `r colFmt("DATASETNAME","red")` %>% mutate(`r colFmt("VARIABLENAME","red")` = factor(`r colFmt("VARIABLENAME","red")`))**
