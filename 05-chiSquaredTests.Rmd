...
colnames(m) <- columnHeaders <- c("","**Response Level 1**", 
                   "**Response Level 2**",
                   "**Response Level 3**",
                   " **...**",
                   "**Response Level C**",
...
```{r Figure5-3,fig.cap = "(ref:fig5-3)",echo = FALSE}
...
```{r Figure5-4,fig.cap = "(ref:fig5-4)",echo = FALSE}
...
par(mai = c(1.5,1.5,0.82,0.42), las = 2, mgp = c(6,1,0))
...
```{r Figure5-6,fig.cap = "(ref:fig5-6)",echo = FALSE}
mosaicplot(chisq.test(electable)$expected,
           main = expression(Plot ~ of ~ H[0]: ~ Independence ~ True))
```
...
```{r Figure5-7,fig.cap = "(ref:fig5-7)",echo = FALSE}
ArthritisFAKE <- rbind(Arthritis,Arthritis) #Just to make the following plot!
ArthritisFAKE$Treat <- factor(c(rep("Placebo",84),rep("Treated",84))) #Just to make the following plot
par(mai = c(1.5,1.5,0.82,0.42), las = 2, mgp = c(6,1,0))
plot(Improved ~ Treat,data = ArthritisFAKE,main = "Homogeneity Null Hypothesis True")
...
cell11 <- "$\\boldsymbol{\\dfrac{n_{\\text{Placebo}\\bullet}*n_{\\bullet\\text{None}}}{N}}$ $\\boldsymbol{ = \\dfrac{43*42}{84}}$ $\\boldsymbol{ = \\color{red}{\\mathbf{21.5}}}$"
cell12 <- "$\\boldsymbol{\\dfrac{n_{\\text{Placebo}\\bullet}*n_{\\bullet\\text{Some}}}{N}}$ $\\boldsymbol{ = \\dfrac{43*14}{84}}$ $\\boldsymbol{ = \\color{red}{\\mathbf{7.167}}}$"
cell13 <- "$\\boldsymbol{\\dfrac{n_{\\text{Placebo}\\bullet}*n_{\\bullet\\text{Marked}}}{N}}$ $\\boldsymbol{ = \\dfrac{43*28}{84}}$ $\\boldsymbol{ = \\color{red}{\\mathbf{14.33}}}$"
cell14 <- "$\\boldsymbol{n_{\\text{Placebo}\\bullet} = 43}$"
row1 <- c("**Placebo**",cell11,cell12,cell13,cell14)

cell21 <- "$\\boldsymbol{\\dfrac{n_{\\text{Treated}\\bullet}*n_{\\bullet\\text{None}}}{N}}$ $\\boldsymbol{ = \\dfrac{41*42}{84}}$ $\\boldsymbol{ = \\color{red}{\\mathbf{20.5}}}$"
cell22 <- "$\\boldsymbol{\\dfrac{n_{\\text{Treated}\\bullet}*n_{\\bullet\\text{Some}}}{N}}$ $\\boldsymbol{ = \\dfrac{41*14}{84}}$ $\\boldsymbol{ = \\color{red}{\\mathbf{6.83}}}$"
cell23 <- "$\\boldsymbol{\\dfrac{n_{\\text{Treated}\\bullet}*n_{\\bullet\\text{Marked}}}{N}}$ $\\boldsymbol{ = \\dfrac{41*28}{84}}$ $\\boldsymbol{ = \\color{red}{\\mathbf{13.67}}}$"
...
```{r Figure5-8,fig.cap = "(ref:fig5-8)",echo = FALSE}
set.seed(98765)
Arthperm <- Arthritis
Arthperm$PermTreatment <- factor(shuffle(Arthperm$Treatment))
par(mfrow = c(2,2))
old.par <- par(mai = c(0.75,0.75,0.5,0.5), las = 2, mgp = c(3.2,1,0))
...
```{r Figure5-9,fig.cap = "(ref:fig5-9)"}
...
```{r Figure5-12,fig.cap = "(ref:fig5-12)",echo = F}
...
```{r Figure5-13,fig.cap = "(ref:fig5-13)"}
...
```{r Figure5-14,fig.cap = "(ref:fig5-14)"}
election2 <- election %>%
...
3. **Calculate the test statistic and p-value:**
    ...
    * The parametric p-value is < 2.2e-16 from the R output which 
    ...
    ```{r Figure5-15,fig.cap = "Plot of $\\boldsymbol{\\chi^2}$-distribution with 12 degrees of freedom.",echo = F}
    ...
    * But since the expected cell count condition is violated, we should 
    ...
    ```{r Figure5-16,fig.cap = "Permutation distribution of $X^2$ for the election data.", warning = F}
...
```{r Figure5-19,fig.cap = "(ref:fig5-19)",echo = F}
...
```{r Figure5-20,fig.cap = "(ref:fig5-20)"}
...
3. **Calculate the test statistic and p-value:**
    ...
    * The parametric p-value is 0.0102 from the R output. This was based
    ...
    ```{r Figure5-21,fig.cap = "Plot of $\\boldsymbol{\\chi^2}$-distribution with 4 degrees of freedom.",echo = F}
...
4. **Conclusion:**
    ...
    * There is strong evidence against the null hypothesis of no relationship
    ...
    ($X^2$-statistic = 13.24, permutation p-value of `r pval`), so conclude that
...
```{r Figure5-23,fig.cap = "(ref:fig5-23)",echo = F}
library(survey)
data(api)
...
```{r Figure5-24,fig.cap = "(ref:fig5-24)"}
...
```{r Figure5-25,fig.cap = "(ref:fig5-25)",echo = F}
old.par <- par(mai = c(1.02,1.5,0.5,0.5), las = 2, mgp = c(6,1,0))
...
```{r Figure5-26,fig.cap = "(ref:fig5-26)",echo = F}
old.par <- par(mai = c(0.5,0.5,0.5,0.5))
...
* **tableplot(data = `r colFmt("DATASETNAME","red")`, sortCol = `r colFmt("VARIABLENAME","red")`, pals = list("BrBG"))**
    ...
    * Makes a tableplot sorted by `r colFmt("VARIABLENAME","red")`, requires
...
tableplot(csd, select = c(isare, careabout, thoughtabout, Gender,
          ...
          sortCol = careabout, pals = list("BrBG")) 
```
...
```{r Figure5-28,fig.cap = "(ref:fig5-28)",echo = F}
...
par(mai = c(1.5,1.5,0.82,0.42), las = 2, mgp = c(5,1,0))
plot(Close ~ Condition2, data = dd, main = "Plot of proportions of close calls by type of clothing")
```
