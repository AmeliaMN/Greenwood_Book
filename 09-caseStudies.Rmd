...
```{r Figure9-1,fig.cap="(ref:fig9-1)",echo=F}
knitr::include_graphics("chapter9_files/decisiontree.jpg")
```
...
```{r echo=F}
gdn <- read_csv("gundalebachnordin_2.csv")
```
...
```{r Figure9-3,fig.cap="(ref:fig9-3)",echo=F, fig.height=9}
...
```{r Figure9-4,fig.cap="(ref:fig9-4)"}
...
```{r Figure9-5,fig.cap="(ref:fig9-5)", fig.height=9}
...
```{r Figure9-6,fig.cap="(ref:fig9-6)"}
...
```{r echo=F}
sasakipratt <- read_csv("sasakipratt.csv")
```
...
```{r echo=F}
bm <- read_csv("bensonmanion.csv")
```

(ref:fig9-10) Scatterplot of *log-biodiversity* vs *log-DBCs* by *TJK*.

```{r Figure9-10,fig.cap="(ref:fig9-10)",echo=T}
...
```{r Figure9-11,fig.cap="(ref:fig9-11)"}
par(mfrow = c(2,2), oma = c(0,0,2,0))
plot(bd1, pch = 16)
```
...
```{r Figure9-12,fig.cap="(ref:fig9-12)"}
...
```{r echo=F}
epworthdata <- read_csv("epworthdata.csv")
```
...
```{r Figure9-13,fig.cap="(ref:fig9-13)"}
...
```{r Figure9-14,fig.cap="(ref:fig9-14)",echo=F}
set.seed(111)
epworthdata$SimEpworth <- simulate(lm_int)[[1]]

epworthdata %>% 
  ggplot(mapping = aes(x = Time, y = SimEpworth, group = Subject, color = Group)) +
  geom_point() +
  geom_line() + 
  theme_bw() + 
  scale_color_colorblind() +
  labs(title = "Simulated Epworth responses")
```
...
```{r Figure9-15,fig.cap="(ref:fig9-15)",echo=F}
plot(allEffects(lme_int), multiline = T, lty = c(1,2), ci.style = "bars", grid = T)
```
